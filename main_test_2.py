...
while episode_cnt < episode_num:
    ...
    while step_cnt < step_num:
        ...
        if not system.render_obj.pause:
            ...
            # add ons
            ########
            # train uav with coord constraint
            reward_1 = reward
            reward_2 = reward
            if episode_cnt < 100:
                user_list = copy.deepcopy(system.user_list)
                x_c, y_c = 0, 0
                dists = []
                for user in user_list:
                    ...
                    dist = (system.UAV.coordinate[0] - coord[0]) ** 2 + (system.UAV.coordinate[1] - coord[1]) ** 2
                    dist = np.sqrt(dist)
                    dists.append(dist)
                
                x_c = x_c / len(user_list) 
                y_c = x_c / len(user_list) 
                dist_max = max(dists) # to normalize penalty for reward 2
                
                penalty = (system.UAV.coordinate[0] - x_c) ** 2 + (system.UAV.coordinate[1] - y_c) ** 2
                penalty = np.sqrt(penalty)
                penalty = penalty / dist_max
                # if the distance away from centroid is within 0.3
                if penalty < 0.3:
                    penalty = 0
                else:
                    penalty = 0.1 * -3 *(penalty - 0.3) ** 2
                reward_2 += penalty
            ...
            # reset memory
                ...
                for agent in [agent_1, agent_2]:
                    ...
                    agent.memory.state_memory[:] = 0
                    agent.memory.new_state_memory[:] = 0
                    agent.memory.action_memory[:] = 0
                    agent.memory.reward_memory[:] = 0
                    agent.memory.terminal_memory[:] = 0

            # 4 store state pair into mem pool
            agent_1.remember(observersion_1, action_1, reward_1, new_state_1, int(done))
            agent_2.remember(observersion_2, action_2, reward_2, new_state_2, int(done))
            # 5 update DDPG net
            agent_1.learn()
